    ...
    def process_groups(self, df: pd.DataFrame, max_samples: Optional[int] = None):
        ...
        for group_idx, group_id in enumerate(unique_groups):
            ...
            for dc_num in [1, 2]:
                ...
                # Reshape voltage and current data
                voltage_data = dc_data[dc_data['Measurement'] == 'Voltage'].copy() if 'Measurement' in dc_data.columns else None
                current_data = dc_data[dc_data['Measurement'] == 'Current'].copy() if 'Measurement' in dc_data.columns else None
                
                if voltage_data is None or current_data is None or voltage_data.empty or current_data.empty:
                    continue
...
def test_saved_model_on_new_data(
    ...
    try:
        ...
        X, group_ids, dc_systems, timestamps = loader.process_groups(df, max_samples)
