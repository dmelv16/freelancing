...
class NewFormatDataLoader:
    ...
    def process_groups(self, df: pd.DataFrame, max_samples: Optional[int] = None, 
        ...
        Args:
            ...
            voltage_only: If True, synthesize current data from voltage
        """
        ...
        if 'Measurement' in df.columns:
            ...
            if not has_current:
                print("WARNING: No current data found in file, will synthesize from voltage")
                voltage_only = True
        ...
        if voltage_only:
            print("Running in VOLTAGE-ONLY mode - synthesizing current data")
        ...
        for group_idx, group_id in enumerate(unique_groups):
            ...
            for dc_num in [1, 2]:
                ...
                if 'DC' in group_data.columns:
                    ...
                    # If no DC column, try to infer from data
                ...
                if voltage_only:
                    # Synthesize current from voltage
                    if 'Timestamp' in voltage_data.columns:
                        ...
                        voltage = voltage_data['Value'].values
                        timestamps = np.arange(len(voltage))
                    
                    # Synthesize current data based on voltage patterns
                    current = self._synthesize_current_from_voltage(voltage, dc_num)
                    ...
                    if current_data is None or current_data.empty:
                        ...
                        if 'Timestamp' in voltage_data.columns:
                            ...
                            # No timestamp, just use values in order
                            voltage = voltage_data['Value'].values[:min(len(voltage_data), len(current_data))]
                            current = current_data['Value'].values[:min(len(voltage_data), len(current_data))]
                            timestamps = np.arange(len(voltage))
    ...
    def _synthesize_current_from_voltage(self, voltage, dc_num):
        """Synthesize current data from voltage patterns
        
        This creates reasonable current values based on voltage patterns:
        - Low voltage = low current (de-energized)
        - Changing voltage = variable current (stabilizing)
        - Steady voltage = steady current (steady state)
        """
        # Base current proportional to voltage (simple Ohm's law assumption)
        # Assuming some nominal resistance
        nominal_resistance = 10.0  # Ohms (adjust based on your system)
        
        # Calculate base current
        current = voltage / nominal_resistance
        
        # Add some realistic noise
        noise_level = 0.05 * np.abs(current.mean()) if current.mean() != 0 else 0.1
        current += np.random.normal(0, noise_level, len(current))
        
        # Add dynamic behavior based on voltage changes
        voltage_diff = np.diff(voltage, prepend=voltage[0])
        
        # When voltage is changing rapidly, current might lag or overshoot
        for i in range(1, len(current)):
            if abs(voltage_diff[i]) > 0.1 * abs(voltage[i]):  # Significant change
                # Add transient response
                current[i] += 0.2 * voltage_diff[i] / nominal_resistance
        
        # Ensure current is non-negative for DC systems
        current = np.maximum(current, 0)
        
        # Add DC-specific characteristics
        if dc_num == 1:
            current *= 1.05  # DC1 might draw slightly more current
        else:
            current *= 0.95  # DC2 might be more efficient
...
class ModelTester:
    ...
    def visualize_predictions(self, num_groups_to_plot: int = 5, use_plotly: bool = False):
        ...
        groups_to_plot = unique_groups[:min(num_groups_to_plot, len(unique_groups))]
