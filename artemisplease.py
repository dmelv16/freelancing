...
def process_single_group_complete(group_df, group_values, group_cols, voltage_cols, test_case_folder):
    ...
    for idx, col in enumerate(voltage_cols):
        ...
        # Calculate features with proper 3, 5, 9 point rolling averages AND slopes
        ...
        try:
            ...
            n_segments = min(5, max(2, n_points // 5000))
            
            # Always use all data for segmentation
            segmenter = GreedyGaussianSegmenter(n_segments=n_segments)
            segments = segmenter.fit_predict(features.reshape(-1, 1))
...
def calculate_complete_features(data):
    ...
    # Combine all features with equal weighting for averages and slopes
    # 6 features total: 3 averages + 3 slopes
    features = (0.2 * avg_3 + 0.2 * avg_5 + 0.15 * avg_9 + 
                0.2 * slope_3 + 0.15 * slope_5 + 0.1 * slope_9)
...
def visualize_all_points(axes, voltage_data, features, segments, col_name, n_points):
    ...
    ax2.plot(time_index, features, alpha=0.6, linewidth=0.5, 
             color='darkgreen', label='Combined Features', zorder=1)
    
    # Overlay segments on features
    for seg_id, color in zip(unique_segments, colors):
        mask = segments == seg_id
        seg_indices = time_index[mask]
        ax2.scatter(seg_indices, features[mask],
                   alpha=0.4, s=0.5, c=[color], label=f'Cluster {seg_id}', zorder=2)
    
    ax2.set_title(f'{col_name} - Features (Avg & Slope: 3, 5, 9 points)')
