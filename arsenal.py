...
class VoltageSegmentAnalyzer:
    ...
    def run_analysis(self, return_only=False):
        ...
        results_dir = self.dc_folder / 'results'
        plots_dir = results_dir / 'plots' / dc_name
...
def main():
    ...
    # Process DC1
    print("=" * 60)
    print("Processing DC1 folder...")
    print("=" * 60)
    dc1_analyzer = VoltageSegmentAnalyzer('greedygaussv6/dc1', cache_thresholds=True)
    dc1_results, dc1_thresholds = dc1_analyzer.run_analysis()
    ...
    # Process DC2
    print("\n" + "=" * 60)
    print("Processing DC2 folder...")
    print("=" * 60)
    dc2_analyzer = VoltageSegmentAnalyzer('greedygaussv6/dc2', cache_thresholds=True)
    dc2_results, dc2_thresholds = dc2_analyzer.run_analysis()
    ...
    # Save combined Excel report at the greedygaussv6 level
    combined_excel_path = Path('greedygaussv6') / 'combined_flagged_analysis.xlsx'
    
    with pd.ExcelWriter(combined_excel_path, engine='openpyxl') as writer:
        # All results from both DC1 and DC2
        ...
        # Statistics by DC folder and label
        if 'dc_folder' in combined_df.columns and 'label' in combined_df.columns:
            stats_by_dc_label = combined_df.groupby(['dc_folder', 'label']).agg({
            ...
            stats_by_dc_label.to_excel(writer, sheet_name='Statistics_by_DC_Label')
        
        # DC1 only flagged
        if 'dc1' in combined_df['dc_folder'].values:
            dc1_flagged = flagged_df[flagged_df['dc_folder'] == 'dc1']
            if not dc1_flagged.empty:
                dc1_flagged.to_excel(writer, sheet_name='DC1_Flagged', index=False)
        
        # DC2 only flagged
        if 'dc2' in combined_df['dc_folder'].values:
            dc2_flagged = flagged_df[flagged_df['dc_folder'] == 'dc2']
            if not dc2_flagged.empty:
                dc2_flagged.to_excel(writer, sheet_name='DC2_Flagged', index=False)
    ...
    if not flagged_df.empty:
        ...
        if 'dc1' in combined_df['dc_folder'].values:
            print(f"  - DC1 flagged: {len(flagged_df[flagged_df['dc_folder'] == 'dc1'])}")
        if 'dc2' in combined_df['dc_folder'].values:
            print(f"  - DC2 flagged: {len(flagged_df[flagged_df['dc_folder'] == 'dc2'])}")
    
    if not combined_thresholds.empty:
        print("\nDynamic Threshold Summary:")
        print(combined_thresholds.groupby(['dc_folder', 'label', 'metric'])['threshold'].mean().round(3))
...
if __name__ == "__main__":
    ...
    # pip install pandas numpy plotly scipy tqdm kaleido openpyxl
    main()
