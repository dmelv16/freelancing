...
class PowerDataClusteringPipeline:
    ...
    def get_feature_columns(self, dc_channel: str = 'combined') -> List[str]:
        """
        Get list of feature columns for clustering
        
        Args:
            dc_channel: 'dc1', 'dc2', or 'combined' for both
        """
        if dc_channel == 'dc1':
            # Only DC1 features
            feature_patterns = [
    ...
    def train_hierarchical_models(self, 
        ...
        Memory-efficient version that processes and saves models incrementally
        ...
        Args:
            ...
            memory_efficient: If True, store only essential model components to save memory
            batch_save: If True, save models immediately after training and keep only references
        """
        ...
        if memory_efficient:
            print("MEMORY EFFICIENT MODE - Storing only essential components")
        
        # Create primary groups
        existing_primary = [col for col in primary_grouping if col in self.df.columns]
        
        # Initialize model storage with memory-efficient structure
        ...
        # Model metadata for lightweight storage
        self.model_metadata = {
            'primary': {},
            'test_case': {}
        }
        ...
        for dc_channel in dc_channels:
            ...
            if 'test_case' in self.df.columns:
                ...
                for test_case in test_cases:
                    ...
                    test_data = self.df[self.df['test_case'] == test_case]
                    ...
                    if len(test_data) >= min_points_for_model:
                        ...
                        model_info = self._train_single_model_memory_efficient(
                            test_data.copy(), 
                            model_key, 
                            dc_channel, 
                            k_max, 
                        ...
                        if model_info:
                            if batch_save:
                                # Save immediately and store only reference
                                self._save_model_batch(model_info, 'test_case', model_key)
                                # Store only metadata
                                self.model_metadata['test_case'][model_key] = {
                                    'path': self.models_dir / f"model_{model_key}.pkl",
                            ...
                            if models_trained % 5 == 0:
                                import gc
                                gc.collect()
                                print(f"  Memory cleanup after {models_trained} models")
                    
                    # Clear test_data from memory
                    del test_data
            ...
            if len(existing_primary) > 0:
                ...
                print(f"\nTraining {len(group_keys)} primary models...")
                ...
                for i, group_key in enumerate(group_keys):
                    ...
                    if len(group_data) >= min_points_for_model:
                        print(f"\n[{i+1}/{len(group_keys)}] Training primary model for: {group_key_str}")
                        ...
                        if model_info:
                            if batch_save:
                                # Save immediately and store only reference
                                self._save_model_batch(model_info, 'primary', group_key_str)
                                # Store only metadata
                                self.model_metadata['primary'][group_key_str] = {
                                    'path': self.models_dir / f"model_{group_key_str}.pkl",
                                    'n_clusters': model_info['n_clusters'],
                                    'n_points': model_info['n_points'],
                                    'dc_channel': dc_channel
                                }
                    ...
                    if (i + 1) % 10 == 0:
                        import gc
                        gc.collect()
                        print(f"  Memory cleanup after {i+1}/{len(group_keys)} groups")
        ...
        if batch_save:
            print(f"  Primary models trained: {len(self.model_metadata.get('primary', {}))}")
            print(f"  Test case fallback models: {len(self.model_metadata.get('test_case', {}))}")
        else:
    ...
    def _train_single_model_memory_efficient(self, data: pd.DataFrame, identifier: str, dc_channel: str, 
                                            k_max: int = 5, max_shuffles: int = 1, 
                                            memory_efficient: bool = True) -> Dict:
        """
        Train a single model with memory-efficient options
        """
        try:
            # Get appropriate feature columns based on DC channel
            feature_cols = self.get_feature_columns(dc_channel)
            ...
            # Clear features_scaled from memory
            del features_scaled
            ...
            # Analyze clusters (lightweight version if memory_efficient)
            if memory_efficient:
                cluster_mapping = self._analyze_clusters_lightweight(data, identifier, dc_channel)
            else:
            ...
            # If memory efficient, don't store the full data
    ...
    def _analyze_clusters_lightweight(self, data: pd.DataFrame, identifier: str, dc_channel: str) -> Dict:
        """
        Lightweight cluster analysis that doesn't store full data
        """
        cluster_stats = []
        
        # Determine appropriate columns based on DC channel
        if dc_channel == 'dc1':
            ...
            voltage_col = 'voltage_28v_dc1_cal' if 'voltage_28v_dc1_cal' in data.columns else 'voltage_28v_dc2_cal'
            slope_col = 'slope_3_V_dc1' if 'slope_3_V_dc1' in data.columns else 'slope_3_V_dc2'
        ...
        for cluster in sorted(data[cluster_col].unique()):
            cluster_mask = data[cluster_col] == cluster
            ...
            stats['avg_position'] = cluster_mask.values.nonzero()[0].mean() / len(data)
        ...
        for _, row in stats_df.iterrows():
            cluster = row['cluster']
            voltage = row.get('avg_voltage', 0)
            slope = row.get('avg_slope', 0)
            
            if voltage < 1.0:
                mapping[cluster] = 'zero_V'
            elif abs(slope) < 0.1 and voltage > 20:
                mapping[cluster] = 'high_V_stable'
            elif slope > 0.1:
                mapping[cluster] = 'increasing'
            elif slope < -0.1:
                mapping[cluster] = 'decreasing'
            else:
    ...
    def _save_model_batch(self, model_info: Dict, level: str, key: str):
        """
        Save model immediately to disk and clear from memory
        """
        model_path = self.models_dir / f"model_{key}.pkl"
        ...
        essential_model = {
            ...
            'n_clusters': model_info['n_clusters'],
            'n_points': model_info['n_points'],
            'dc_channel': model_info.get('dc_channel', 'combined')
        }
    ...
    def load_model_for_prediction(self, model_key: str):
        """
        Load a specific model from disk when needed
        """
        # Check metadata first
        for level in ['primary', 'test_case']:
            if model_key in self.model_metadata.get(level, {}):
                model_path = self.model_metadata[level][model_key]['path']
        ...
        for level in ['primary', 'test_case']:
            if level in self.hierarchical_models and model_key in self.hierarchical_models[level]:
                return self.hierarchical_models[level][model_key]
    ...
    def _train_single_model_dc(self, data: pd.DataFrame, identifier: str, dc_channel: str, k_max: int = 5, max_shuffles: int = 1) -> Dict:
        ...
        """
        try:
            # Get appropriate feature columns based on DC channel - FIXED to properly separate
            feature_cols = self.get_feature_columns(dc_channel)
            
            # Make sure we have features and they exist in data
            feature_cols = [col for col in feature_cols if col in data.columns]
            if not feature_cols:
                print(f"  No features found for {dc_channel} in data")
                return None
            ...
            print(f"  Successfully trained {dc_channel} model with {model_info['n_clusters']} clusters")
            print(f"  Features used: {feature_cols[:3]}..." if len(feature_cols) > 3 else f"  Features used: {feature_cols}")
            return model_info
